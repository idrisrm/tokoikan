                ...
                <div class="container-fluid mt-n10">
                    <div class="card card-header-actions mx-auto mb-4">
                        ...
                        <div class="card-header">
                            ...
                            <div class="no-caret">
                                <a class="btn btn-sm btn-primary" target="_blank" href="<?= base_url(); ?>export/getPengaduanOPD/proses/">
                                    ...
                                    <tbody id="target">
                                        <!-- <?php
                                            ...
                                            <tr>
                                                ...
                                                <td><?= $d['nm_kategori'] ?></td>
                                                <td>
                                                    <div class="badge badge-warning badge-pill"><?= $d['st_pengaduan'] ?></div>
                                                </td>
                                                <td>
                                                    <a class="btn btn-datatable btn-icon btn-transparent-dark" href="<?php echo base_url('PengaduanOPD/detail_arsip/' . $d['id_pengaduan']) ?>"><i data-feather="zoom-in"></i></a>
                                                </td>
                                            </tr>
                                        <?php } ?> -->
                                    </tbody>
...
<script>
    $(document).ready(function() {
        ...
        $("#kategori").select2({
            ...
            idKey = $("#kategori").val();
            console.log(idKey);
            $('.print_kategori').attr('href', '<?= base_url(); ?>export/getPengaduanOPD/arsip/' + idKey);
            getFilter(idKey);
        });
    });
    $.ajax({
        url: "<?php echo base_url() . "PengaduanOPD/data_arsip_ajax" ?>",
        dataType: "json",
        success: function(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                baris += '<tr>' +
                    ...
                    '<td> <a href="detail_arsip/' + data[i].id_arsip + '" class="btn btn-datatable btn-icon btn-transparent-dark"><i class="fa fa-search-plus"></i></a></td>'
                '</tr>'
            }
    ...
    function getFilter(idKey) {
        $.ajax({
            url: "<?= base_url(); ?>Pengaduan/getFilterArsipOPD",
            method: "POST",
            data: {
                id_kategori: idKey
            },
            ...
            success: function(data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    baris += '<tr>' +
                        ...
                        '<td>' + data[i].nm_opd + '</td>' +
                        '<td>' + data[i].nm_kategori + '</td>' +
                        '<td>' + '<div class="badge badge-warning badge-pill">' + data[i].st_pengaduan + '</div>' + '</td>' +
                        '<td> <a href="detail_arsip/' + data[i].id_arsip + '" class="btn btn-datatable btn-icon btn-transparent-dark"><i class="fa fa-search-plus"></i></a></td>'
                    '</tr>'
                }
                $('#dataTable').dataTable().fnDestroy();
