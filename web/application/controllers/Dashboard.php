...
class Dashboard extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $jember =  $this->db->query("SELECT COUNT(id_detail) as count,MONTHNAME(detail_penjualan.created_at) as month_name FROM detail_penjualan, penjualan WHERE penjualan.id_otlet = 1 AND penjualan.pengajuan = '' AND detail_penjualan.id_penjualan = penjualan.id_penjualan AND YEAR(detail_penjualan.created_at) = '" . date('Y') . "' GROUP BY YEAR(detail_penjualan.created_at), MONTH(detail_penjualan.created_at)")->result_array();
        ...
        $situbondo =  $this->db->query("SELECT COUNT(id_detail) as count,MONTHNAME(detail_penjualan.created_at) as month_name FROM detail_penjualan, penjualan WHERE penjualan.id_otlet = 2 AND penjualan.pengajuan = '' AND detail_penjualan.id_penjualan = penjualan.id_penjualan AND YEAR(detail_penjualan.created_at) = '" . date('Y') . "' GROUP BY YEAR(detail_penjualan.created_at), MONTH(detail_penjualan.created_at)")->result_array();
        ...
        ///grafik Bali
        $bali =  $this->db->query("SELECT COUNT(id_detail) as count,MONTHNAME(detail_penjualan.created_at) as month_name FROM detail_penjualan, penjualan WHERE penjualan.id_otlet = 3 AND penjualan.pengajuan = '' AND detail_penjualan.id_penjualan = penjualan.id_penjualan AND YEAR(detail_penjualan.created_at) = '" . date('Y') . "' GROUP BY YEAR(detail_penjualan.created_at), MONTH(detail_penjualan.created_at)")->result_array();
